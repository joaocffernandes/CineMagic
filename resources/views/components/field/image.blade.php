...
<div {{ $attributes }}>
    <div class="flex-col">
        ...
        <img id="image" class="{{$widthClass}} {{$maxHeightClass}} aspect-auto" src="{{ $imageUrl }}">
        @if(!$readonly)
        <div class="{{$widthClass}} flex-col space-y-4 items-stretch mt-4">
            <div>
                <div class="flex flex-row items-center">
                    <input id="id_{{ $name }}" name="{{ $name }}" type="file" accept="image/png, image/jpeg" onchange="(document.getElementById('id_{{ $name }}_selected_file').innerHTML= document.getElementById('id_{{ $name }}').files[0].name ?? '') && readURL(this);" class="hidden" />
                    <label for="id_{{ $name }}" class="min-w-32
                    ...
                    <label id="id_{{ $name }}_selected_file" class="text-sm text-slate-500 truncate"></label>
                </div>
                @error( $name )
            ...
            @if($deleteAllow)
            <div>
                <x-button element="submit" :text="$deleteTitle" type="danger" form="{{$deleteForm}}" />
            </div>
            @endif
        </div>
        @endif
