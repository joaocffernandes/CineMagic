...
<div {{ $attributes->merge(['class' => "$widthClass"]) }}>
    ...
    <textarea id="id_{{ $name }}" name="{{ $name }}" class="appearance-none block
            ...
            {{ !$resizable || $readonly ? 'resize-none' : 'resize-y' }}" @required($required) @disabled($readonly) autofocus="autofocus">
    {{ $value }}
    </textarea>
    @error( $name )
