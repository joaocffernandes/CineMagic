<div {{ $attributes }}>
    <table class="table-auto border-collapse">
        ...
        <tr class="border-b-2 border-b-gray-400 dark:border-b-gray-500 bg-gray-100 dark:bg-gray-800">
            ...
            <th class="px-2 py-2 text-left">Seat Number</th>
        ...
        @foreach ($tickets as $ticket)
            <tr class="border-b border-b-gray-400 dark:border-b-gray-500">
                ...
                <td class="px-2 py-2 text-center">{{ $ticket->seat->seat_number }}</td>
                ...
                @if($showAddToCart)
                    <td>
                        <x-table.icon-add-cart class="px-0.5"
                            method="post"
                            action="{{ route('cart.add', ['ticket' => $ticket->id]) }}"/>
                    </td>
                @endif
                @if($showRemoveFromCart)
                    <td>
                        <x-table.icon-minus class="px-0.5"
                            method="delete"
                            action="{{ route('cart.remove', ['ticket' => $ticket->id]) }}"/>
                    </td>
                @endif
            </tr>
        @endforeach
